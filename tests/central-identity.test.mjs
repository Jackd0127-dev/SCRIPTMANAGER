...
test("creates a five minute ScriptAI identity proof", () => {
  ...
  const token = createLegacyLinkProof({ legacyUserId: "script-user", email: "owner@example.test", privateKey: pem, issuer: "https://scriptai.space", audience: "https://access.novasagency.com/identity-link", keyId: "test", now: 100 });
  ...
  assert.equal(claims.email, "owner@example.test");
